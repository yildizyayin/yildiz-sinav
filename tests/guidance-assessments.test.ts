...
describe('counselor-governed guidance assessments',()=>{
 ...
 it('maps explicit student test requests to governed instrument codes',()=>{
 ...
 it('keeps positive error-review wording semantically distinct from error repetition',()=>{
  const result=scoreGuidanceResponses({scale:{min:1,max:5},items:[{id:'r5',dimension:'error_review',text:'Yanlışlarımı incelerim.'}]},{r5:5});
  expect(result.dimensions.error_review).toBe(100);
  expect(result.dimensions).not.toHaveProperty('error_repetition');
 });
});
