...
export function ExamDefinitions() {
  ...
  const analyseKey = (text = answerKeyText) => {
    ...
    setCreateForm((current) => ({ ...current, title: current.title || result.metadata?.title || '', publisherName: current.publisherName || result.metadata?.publisherName || '', gradeLevel: current.gradeLevel || String(result.metadata?.gradeLevel || selectedChoice.gradeLevel || ''), description: current.description || (result.detectedFormat === 'WIDE_BOOKLET_TABLE' ? 'Kazanımlı cevap anahtarından otomatik oluşturuldu; yayın öncesi kontrol edilmelidir.' : '') }));
