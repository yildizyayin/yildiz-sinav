...
export function StudentBooks(){
 ...
 const load=async()=>{try{const[p,z,e]=await Promise.all([api<any>('/api/student-books/personal'),api<any>('/api/student-books/zero-error'),api<any>('/api/platform/exam-center/results')]);setPersonal(p.books||[]);setZero(z.booklets||[]);setExams(e.results||[])}catch(e:any){setError(e.message)}};
 useEffect(()=>{void load()},[]);
 ...
 return <>
  ...
  <div className="student-top-grid">
   ...
   <div className="goal-story-card"><div className="goal-story-icon"><Target/></div><span>Sıfır Hata Kitapçığı</span><h2>Yanlış ve boşları kapat</h2><p>Seçtiğin sınavdaki yanlış/boş soruların kazanımları üzerinden benzer sorularla tekrar döngüsü oluşturur.</p><div style={{display:'flex',gap:8,flexWrap:'wrap'}}><select value={selectedExam} onChange={e=>setSelectedExam(e.target.value)} style={{flex:1,minWidth:220}}><option value="">Sınav seç</option>{exams.map((e:any)=><option key={`${e.id}-${e.snapshot_version}`} value={e.id}>{e.title}</option>)}</select><button className="primary" disabled={busy||!selectedExam} onClick={()=>void createZero()}><RefreshCcw size={16}/> Kitapçığı Oluştur</button></div></div>
  </div>
