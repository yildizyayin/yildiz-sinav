...
export function ResultNetworkAdmin(){
 ...
 return <>
  ...
  <section className="panel" style={{marginTop:18}}><div className="panel-head"><div><h2>Yıl → sınav arşivi ve yayınlar</h2><p>Önce değerlendirmeye açın; tüm kurum yüklemeleri bitince sıralamayı dondurup öğrenci sonuçlarını yayınlayın.</p></div><button className="ghost" onClick={()=>void load()}><RefreshCw/> Yenile</button></div><div className="cards-list">{(data.administrations||[]).map((x:any)=><div className="list-card" key={x.id}><div><strong>{x.academic_year} · {x.publisher_name?`${x.publisher_name} · `:''}{x.title}</strong><span>{x.exam_type} · {Number(x.result_count||0).toLocaleString('tr-TR')} değerlendirme · {x.linked_institutions} kurum · {x.access_count} öğrenci erişimi</span><small>Durum: {x.status}{x.ranking_frozen_at?' · Sıralama donduruldu':''}{x.retention_due_at?` · Kişisel veri silinme: ${new Date(x.retention_due_at).toLocaleDateString('tr-TR')}`:''}</small></div><div style={{display:'flex',gap:8,flexWrap:'wrap'}}>{x.status==='DRAFT'&&<button className="primary" disabled={busy} onClick={()=>void publish(x.id)}>Değerlendirmeye aç</button>}{['UPLOADING','READY','PUBLISHED'].includes(x.status)&&!x.ranking_frozen_at&&<button className="primary" disabled={busy||!Number(x.result_count)} onClick={()=>void finalize(x)}>Sıralamayı dondur ve yayınla</button>}</div></div>)}{!data.administrations?.length&&<div className="empty">Henüz sonuç ağına açılmış sınav yok.</div>}</div></section>
 </>;
}
