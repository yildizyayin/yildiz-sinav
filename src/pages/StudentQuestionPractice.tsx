...
import { BookOpen, Check, CheckCircle2, ChevronRight, Lightbulb, RotateCcw, Sparkles, Target, XCircle } from 'lucide-react';
  ...
  return <div className="student-practice-page">
    ...
    {!loading && current && <div className="practice-layout">
      <main className="practice-question-card">
        ...
        <div className="practice-question-content"><span className="eyebrow">{current.topic || 'Kazanım pratiği'}</span><h2>{current.stem_text}</h2><div className="practice-options">{(current.options || []).map((option: any, optionIndex: number) => { const data = optionData(option, optionIndex); const selected = answer === data.letter; const correct = result && result.correct && selected; const wrong = result && !result.correct && selected; return <button className={`practice-option ${selected ? 'selected' : ''} ${correct ? 'correct' : ''} ${wrong ? 'wrong' : ''}`} disabled={!!result} key={`${data.letter}-${optionIndex}`} onClick={() => setAnswer(data.letter)}><span className="practice-option-letter">{data.letter}</span><strong>{data.text}</strong>{correct && <CheckCircle2 size={18} />}{wrong && <XCircle size={18} />}</button>; })}</div></div>
