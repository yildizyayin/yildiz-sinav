...
function StudentGuidance(){
 ...
 return <>
  ...
  {sessions.map(s=><div className="panel" key={s.id} style={{marginBottom:16}}><div className="page-head" style={{marginBottom:8}}><div><span className="eyebrow">{s.category}</span><h2>{s.title}</h2><p>{STATUS[s.status]||s.status}</p></div>{s.status==='REVIEWED'?<CheckCircle2 size={30}/>:<Clock3 size={30}/>}</div>
   ...
   {s.status==='SUBMITTED'&&<div className="alert">Yanıtların gönderildi. Sonuçlar Nibiru tarafından gelişiminde kullanılmadan önce gerçek rehber öğretmenin inceleyecek.</div>}
...
function CounselorGuidance(){
 ...
 const act=async(row:any,op:'approve'|'reject'|'review')=>{try{setBusy(row.id+op);setError('');const method='PATCH';const r=await api<any>(`/api/nibiru/guidance/assessments/${encodeURIComponent(row.id)}/${op}`,{method,body:JSON.stringify({note:notes[row.id]||''})});setNotice(op==='approve'?'Test öğrenciye açıldı.':op==='reject'?'Öneri reddedildi.':r.summary||'Sonuç gelişim profiline kabul edildi.');await load()}catch(e:any){setError(e.message)}finally{setBusy('')}};
