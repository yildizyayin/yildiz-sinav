...
function CounselorGuidance(){
 ...
 const proposed=rows.filter(r=>r.status==='PROPOSED').length,submitted=rows.filter(r=>r.status==='SUBMITTED').length,inProgress=rows.filter(r=>['APPROVED','IN_PROGRESS'].includes(r.status)).length,classCount=new Set(rows.map(r=>r.class_id).filter(Boolean)).size;
 return <>{notice&&<div className="alert success">{notice}</div>}{error&&<div className="alert error">{error}</div>}<div className="alert warning"><ShieldCheck size={18}/><div><strong>İnsan onayı kapısı</strong><div>Nibiru yalnız önerir. Öğrenciye testi açma ve gönderilen sonucu gelişim profiline kabul etme kararı atandığı gerçek rehber öğretmendedir.</div></div></div>
  <div className="kpi-grid" style={{marginBottom:20}}><div className="kpi-card"><span>Onay Bekleyen</span><strong>{proposed}</strong></div><div className="kpi-card"><span>İnceleme Bekleyen</span><strong>{submitted}</strong></div><div className="kpi-card"><span>Uygulama Sürecinde</span><strong>{inProgress}</strong></div><div className="kpi-card"><span>Kuyruktaki Sınıf</span><strong>{classCount}</strong></div></div>
  ...
  {rows.map(r=><div className="panel" key={r.id} style={{marginBottom:16}}><div className="page-head" style={{marginBottom:10}}><div><span className="eyebrow">{r.class_name||'Sınıf'} · {r.category}</span><h2>{r.first_name} {r.last_name}</h2><p>{r.title} · {STATUS[r.status]||r.status}</p></div><ClipboardCheck size={28}/></div><p>{r.proposal_reason}</p>
   ...
   <label>Rehber öğretmen notu {r.status==='SUBMITTED'&&<span className="muted">· profil kabulünde zorunlu</span>}<textarea value={notes[r.id]||''} onChange={e=>setNotes({...notes,[r.id]:e.target.value})} placeholder={r.status==='SUBMITTED'?'Eğitimsel gelişim değerlendirmesini kısaca yazın':'Onay notu isteğe bağlı; reddetme gerekçesi zorunludur'}/></label>
   <div style={{display:'flex',gap:10,flexWrap:'wrap',marginTop:12}}>{r.status==='PROPOSED'&&<><button className="primary" disabled={busy!==''} onClick={()=>void act(r,'approve')}><CheckCircle2 size={16}/> Onayla ve Öğrenciye Aç</button><button className="ghost" disabled={busy!==''||!(notes[r.id]||'').trim()} onClick={()=>void act(r,'reject')}><XCircle size={16}/> Gerekçeyle Reddet</button></>}{r.status==='SUBMITTED'&&<button className="primary" disabled={busy!==''||!(notes[r.id]||'').trim()} onClick={()=>void act(r,'review')}><ShieldCheck size={16}/> İncele ve Gelişim Profiline Kabul Et</button>}</div>
  </div>)}</>;
}
