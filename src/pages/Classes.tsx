...
export function Classes(){
  ...
  return <>
    ...
    <div className="cards-list">{rows.map(r=><div className="list-card" key={r.id}><div className="quick-icon"><Users/></div><div><strong>{r.name}</strong><span>{r.student_count} aktif öğrenci{guidance?' · tüm dersler':' · branş kapsamı'}</span></div><Link className="secondary" to="/reports"><BarChart3 size={15}/> Gelişim</Link></div>)}{!rows.length&&<div className="empty">Henüz atanmış sınıf bulunmuyor.</div>}</div>
