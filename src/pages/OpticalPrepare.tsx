...
export function OpticalPrepare() {
  ...
  useEffect(() => {
    ...
    const ready = calibrations.find(
      (c: any) =>
        c.optical_template_version_id === templateId && c.status === "READY",
    );
    if (ready) setProfileId(ready.printer_profile_id);
  }, [templateId, calibrations, profiles.length]);
  const calibration = useMemo(
    () =>
      calibrations.find(
        (c: any) =>
          c.printer_profile_id === profileId &&
          c.optical_template_version_id === templateId,
      ) || null,
    [calibrations, profileId, templateId],
  );
  ...
  const students = useMemo(
    () => data?.students?.filter((s: any) => selected.includes(s.id)) || [],
    [data, selected],
  );
  const fields: PrintField[] = useMemo(() => {
    const p = data?.template?.printFields;
    if (!p) return [];
    return Array.isArray(p.fields)
      ? p.fields
      : Object.entries(p).map(([key, value]: any) => ({ key, ...value }));
  }, [data]);
  const toggle = (id: string, on: boolean) =>
    setSelected((cur) =>
      on ? [...new Set([...cur, id])] : cur.filter((x) => x !== id),
    );
  const fieldValue = (f: PrintField, s: any) => {
    switch (f.key) {
      case "studentName":
      ...
      case "class":
        return `${s.grade_level || ""}/${s.section || ""}`;
      case "section":
        return s.section || "";
      case "institutionCode":
        return data?.institution?.code || "";
      case "bookletCode":
        return s.booklet_code || "A";
      case "examTitle":
      ...
      case "qr":
      case "barcode":
        return `${data?.exam?.id || "OPT"}:${s.id}`;
      default:
        return "";
    }
  };
  const renderField = (f: PrintField, s: any, i: number) => {
    const left = Number(f.xMm ?? f.x ?? 0),
      top = Number(f.yMm ?? f.y ?? 0);
    const baseStyle: React.CSSProperties = {
      position: "absolute",
      left: `${left}mm`,
      top: `${top}mm`,
      fontSize: `${Number(f.fontSizePt || 9)}pt`,
      fontWeight: 700,
      lineHeight: 1,
    };
    if (f.key === "studentNumberBubbles")
      return (
        <BubbleNumber
          key={`${s.id}-${i}`}
          value={String(s.student_number || "")}
          field={f}
          style={baseStyle}
        />
      );
    return (
      <div key={`${s.id}-${i}`} style={baseStyle}>
        {fieldValue(f, s)}
      </div>
    );
  };
  ...
  return (
    <>
      ...
      <div className="panel optical-controls">
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {templates.map((t) => (
            <button
              key={t.version_id}
              className={templateId === t.version_id ? "primary" : "secondary"}
              onClick={() => setTemplateId(t.version_id)}
            >
              {t.name}’a Bas
            </button>
          ))}
          {!templates.length && (
            <span className="muted">
              Henüz baskı alanı tanımlanmış optik yok.
            </span>
          )}
        </div>
      </div>
      <div className="panel optical-controls">
        <div className="form-grid">
          ...
          <label>
            ...
            >
              <option value="">Baskı tasarımı tamamlanmış optik seç</option>
              {templates.map((t) => (
          ...
          <label>
            ...
            >
              <option value="">Yazıcı profili seç</option>
              {profiles.map((p) => (
          ...
          <label>
            ...
            <select value={examId} onChange={(e) => setExamId(e.target.value)}>
              <option value="">Sınavsız genel optik</option>
              {exams.map((e) => (
          ...
          <label>
            ...
            <input
              value={bookletSet}
              onChange={(e) => setBookletSet(e.target.value.toUpperCase())}
              placeholder="Boş = sınavdan otomatik, örn. A,B,C,D"
            />
          </label>
          <label>
            Sıralama
        ...
        {!profiles.length ? (
          <div className="alert info">
            Bu kurum için yazıcı profili yok.{" "}
            <Link className="link-button" to="/calibration">
              Yazıcı profili oluştur ve kalibre et.
            </Link>
          </div>
        ) : calibrationReady ? (
          <div className="success-box">
            ...
            <div>
              <strong>
                ...
                {templates.find((t) => t.version_id === templateId)?.name} hazır
              </strong>
              <span>
                Kalibrasyon baskıya otomatik uygulanacak: X{" "}
                {correction.correctionOffsetX.toFixed(2)} mm · Y{" "}
                {correction.correctionOffsetY.toFixed(2)} mm · Ölçek{" "}
                {correction.correctionScaleX.toFixed(5)} /{" "}
                {correction.correctionScaleY.toFixed(5)} · Dönüş{" "}
                {correction.correctionRotation.toFixed(3)}°
              </span>
            </div>
          </div>
        ) : profileId && templateId ? (
          <div className="alert warning">
            Bu yazıcı + optik kombinasyonu henüz doğrulanmadı.{" "}
            <Link className="link-button" to="/calibration">
              <Wrench size={14} /> Kalibrasyona Git
            </Link>
          </div>
        ) : null}
        <button
          ...
          style={{ marginTop: 14 }}
          onClick={load}
          disabled={busy || !classId || !templateId}
        >
          <Printer size={17} />
          {busy ? "Hazırlanıyor…" : "Baskıyı Hazırla"}
        </button>
      </div>
      {data && (
        <div className="panel optical-controls">
          <div className="panel-head">
            <div>
              ...
              <p>
                ...
                seçin ve %100 / Actual Size kullanın.
              </p>
            </div>
            <button
              className="primary"
          ...
          >
            <label className="check">
              <input
                type="checkbox"
                checked={selected.length === data.students.length}
                onChange={(e) =>
                  setSelected(
                    e.target.checked ? data.students.map((s: any) => s.id) : [],
                  )
                }
              />{" "}
              Tümünü seç
            ...
            <span className="muted">
              Kitapçık: {(data.bookletCodes || []).join(" / ")}
            </span>
            <span className={`status ${calibrationReady ? "ok" : "warn"}`}>
              {calibrationReady ? "Kalibrasyon hazır" : "Kalibrasyon gerekli"}
            </span>
          </div>
      ...
      {data && (
        <div
          className="optical-print-root"
          style={{ display: students.length ? "block" : "none" }}
        >
          {students.map((s: any) => (
            <div
              className="print-optical-page"
              key={s.id}
              style={{
                width: `${data.template.pageWidthMm}mm`,
                height: `${data.template.pageHeightMm}mm`,
                position: "relative",
                margin: "16px auto",
                background: "#fff",
            ...
            >
              <div
                style={{
                  ...
                  height: `${data.template.pageHeightMm}mm`,
                  transformOrigin: "0 0",
                  transform: calibrationReady
                    ? calibrationTransform(calibration)
                    : "none",
                  backgroundImage: `url(/api/v2/optical-print-base?versionId=${encodeURIComponent(data.template.id)})`,
                  backgroundSize: "100% 100%",
                  backgroundRepeat: "no-repeat",
                }}
              >
                {fields.map((f, i) => renderField(f, s, i))}
              </div>
            </div>
          ))}
        </div>
...
function BubbleNumber({
  ...
  const digits = Math.max(
    Number(field.digits || value.length || 4),
    value.length,
  );
  const padded = value.padStart(digits, "0").slice(-digits);
  const colPitch = Number(field.columnPitchMm || 5);
  const rowPitch = Number(field.rowPitchMm || 4.1);
  return (
    <div style={style}>
      {Array.from({ length: digits }).map((_, col) => (
        <div
          key={col}
          style={{ position: "absolute", left: `${col * colPitch}mm`, top: 0 }}
        >
          {Array.from({ length: 10 }).map((__, n) => (
            <div
              ...
              style={{
                ...
                borderRadius: "50%",
                background: padded[col] === String(n) ? "#111" : "transparent",
              }}
            />
          ))}
        </div>
