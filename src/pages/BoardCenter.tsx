...
import { api } from '../api';
import { ClassSelect } from '../components/EntitySelectors';
...
export function BoardCenter(){
 ...
 <div className="panel" style={{marginBottom:16}}><div className="panel-head"><div><h2>Yeni tahta oturumu</h2><p>İçerik silosu oluşturmaz; soru havuzu, video ve föy motorunu yeniden kullanır.</p></div><MonitorPlay/></div><div className="form-grid"><label>Oturum başlığı<input value={title} onChange={e=>setTitle(e.target.value)} placeholder="8/A Matematik · Günün Soruları"/></label><ClassSelect value={classId} onChange={setClassId}/></div><button className="primary" onClick={create} disabled={!title.trim()}><Plus size={16}/> Oturumu Aç</button></div>
