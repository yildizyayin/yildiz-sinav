...
try{
 ...
 assert(status?.providers?.standardReady===true,'Standard TTS route is not ready',status);
 ...
 console.log(`✓ Nibiru Voice readiness — STT ${status.providers.stt.model} · Standard ${status.plans.standard.providers.join(' → ')}`);

 const probe=await jsonReq('/api/nibiru/voice/probe?mode=standard',{method:'POST',cookie:admin});
 assert(probe?.ok===true&&Number(probe?.bytes||0)>100,'Standard TTS live probe returned no audio',probe);
