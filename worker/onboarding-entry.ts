...
async function createInstitution(request:Request,env:Env,user:AuthUser){
 ...
 const statements:D1PreparedStatement[]=[
  ...
  env.DB.prepare(`INSERT INTO users(id,institution_id,role,display_name,email,username,password_hash,password_salt,password_iterations,password_algo,active) VALUES(?,?,'INSTITUTION_MANAGER',?,?,?,?,?,?,?,'PBKDF2-SHA256-v1',1)`).bind(managerId,institutionId,managerName,managerEmail,managerEmail,passwordData.hash,passwordData.salt,passwordData.iterations),
