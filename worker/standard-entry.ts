...
const unsafeTitlePattern=/(bahis|casino|kumar|slot|silah|şiddet|cinsel|porno|alkol|sigara|uyuşturucu|küfür|hack|hile)/i;
export function youtubeCandidatePolicy(candidate:{title?:string;duration_seconds?:number;relevance_score?:number;embeddable?:boolean;privacy_status?:string;age_restricted?:boolean}){
  const flags:string[]=[];if(!candidate.embeddable)flags.push('NOT_EMBEDDABLE');if(candidate.privacy_status!=='public')flags.push('NOT_PUBLIC');if(candidate.age_restricted)flags.push('AGE_RESTRICTED');if(unsafeTitlePattern.test(String(candidate.title||'')))flags.push('UNSAFE_TITLE');const duration=Number(candidate.duration_seconds||0);if(duration<45||duration>180)flags.push('DURATION_OUT_OF_RANGE');if(Number(candidate.relevance_score||0)<.12)flags.push('LOW_OUTCOME_MATCH');return {passed:flags.length===0,flags};
}
...
async function youtubeMicroVideo(env:Env,question:any){
  ...
  const cached=await all<any>(env.DB.prepare(`SELECT youtube_video_id id,title,channel_title,url,duration_seconds,view_count,relevance_score,popularity_score,ai_selected,human_review_status FROM youtube_micro_video_candidates WHERE exam_question_id=? AND active=1 AND policy_status='PASSED' AND (expires_at IS NULL OR expires_at>datetime('now')) ORDER BY human_review_status='APPROVED' DESC,ai_selected DESC,relevance_score DESC,popularity_score DESC LIMIT 5`).bind(question.question_id));
  if(cached.length){const approved=cached.find(x=>x.human_review_status==='APPROVED');return {video:approved||null,reason:approved?'HUMAN_APPROVED_CACHE':'HUMAN_REVIEW_REQUIRED',candidateCount:cached.length};}
  ...
  let items=(details.items||[]).map((x:any)=>{const duration=parseIsoDuration(x.contentDetails?.duration)||0,views=Number(x.statistics?.viewCount||0),rel=relevance(query,x.snippet?.title||''),ageRestricted=x.contentDetails?.contentRating?.ytRating==='ytAgeRestricted';const item={id:x.id,title:x.snippet?.title||'Konu Anlatımı',channel_id:x.snippet?.channelId||'',channel_title:x.snippet?.channelTitle||'',url:`https://www.youtube.com/watch?v=${x.id}`,duration_seconds:duration,view_count:views,relevance_score:rel,popularity_score:Math.log10(Math.max(views,1)),embeddable:x.status?.embeddable===true,privacy_status:String(x.status?.privacyStatus||''),age_restricted:ageRestricted};return {...item,policy:youtubeCandidatePolicy(item)}}).filter((x:any)=>x.policy.passed);
  ...
  for(const item of items)await env.DB.prepare(`INSERT INTO youtube_micro_video_candidates(id,exam_question_id,outcome_id,grade_level,subject_id,search_query,youtube_video_id,title,channel_id,channel_title,url,duration_seconds,view_count,relevance_score,popularity_score,ai_selected,safe_search,embeddable,privacy_status,age_restricted,policy_status,policy_flags_json,human_review_status,fetched_at,expires_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'PENDING',CURRENT_TIMESTAMP,?,CURRENT_TIMESTAMP)
    ON CONFLICT(exam_question_id,youtube_video_id) DO UPDATE SET outcome_id=excluded.outcome_id,grade_level=excluded.grade_level,subject_id=excluded.subject_id,search_query=excluded.search_query,title=excluded.title,channel_id=excluded.channel_id,channel_title=excluded.channel_title,url=excluded.url,duration_seconds=excluded.duration_seconds,view_count=excluded.view_count,relevance_score=excluded.relevance_score,popularity_score=excluded.popularity_score,ai_selected=excluded.ai_selected,safe_search=excluded.safe_search,embeddable=excluded.embeddable,privacy_status=excluded.privacy_status,age_restricted=excluded.age_restricted,policy_status=excluded.policy_status,policy_flags_json=excluded.policy_flags_json,fetched_at=CURRENT_TIMESTAMP,expires_at=excluded.expires_at,updated_at=CURRENT_TIMESTAMP`)
    .bind(uuid('ytm'),question.question_id,question.outcome_id||null,question.grade_level||null,question.subject_id,query,item.id,item.title,item.channel_id,item.channel_title,item.url,item.duration_seconds,item.view_count,item.relevance_score,item.popularity_score,item.id===selected?.id?1:0,1,item.embeddable?1:0,item.privacy_status,item.age_restricted?1:0,'PASSED',JSON.stringify(item.policy.flags),expires).run();
  return {video:null,reason:'HUMAN_REVIEW_REQUIRED',candidateCount:items.length};
}
