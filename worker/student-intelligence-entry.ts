...
async function nibiruChat(request:Request,env:Env,ctx:ExecutionContext){
 ...
 let payload:any;try{payload=await response.clone().json()}catch{return response}if(typeof payload?.answer==='string')payload.answer=payload.answer.replace(/^\s*Nibiru\s*:/,'Nibiru:');
