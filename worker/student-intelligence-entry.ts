...
async function compactStudentIntelligence(env:Env,studentId:string){
 ...
 if(cached){const payload=parseJson<any>(cached.payload_json,{}),overall=payload?.overall||{},priorities=Array.isArray(payload?.priorities)?payload.priorities.slice(0,3):[],loop=payload?.learningLoop||{};return {studentId,profileVersion:Number(cached.profile_version||1),summary:`Akademik profil: ustalık ${overall.masteryScore??'yetersiz kanıt'}, güven ${Math.round(Number(overall.academicConfidence||0)*100)}%, eğilim ${overall.examTrend||'INSUFFICIENT'}.`,priorities:priorities.map((x:any)=>`${x.subjectName||'Ders'} · ${x.outcomeTitle} (${x.masteryScore}%)`),goalMotivation:payload?.goalMotivation||null,learningLoop:{activeRecoveryCount:Number(loop?.recovery?.activeCount||0),openCoachTaskCount:Number(loop?.tasks?.openPersonalTaskCount||0),pendingFollowupCount:Number(loop?.coach?.pendingFollowupCount||0),openZeroErrorCount:Number(loop?.zeroError?.openBookletCount||0),availableApprovedVideoCount:Number(loop?.videos?.availableApprovedCount||0)},evidenceSources:payload?.evidenceSources||[],policy:payload?.policy||{educationalOnly:true,diagnosticUse:false,rawGuidanceResponsesIncluded:false,targetGapFabricated:false}}}
...
export default {
 async fetch(request:Request,env:Env,ctx:ExecutionContext):Promise<Response>{const p=new URL(request.url).pathname;if(p==='/api/student-intelligence/profile'&&(request.method==='GET'||request.method==='POST'))return profile(request,env) as Promise<Response>;if(p==='/api/student-intelligence/history'&&request.method==='GET')return history(request,env) as Promise<Response>;if(p==='/api/student-intelligence/learning-graph'&&request.method==='GET')return graph(request,env) as Promise<Response>;if(p==='/api/nibiru/chat'&&request.method==='POST')return nibiruChat(request,env,ctx);return app.fetch(request,env,ctx)},
