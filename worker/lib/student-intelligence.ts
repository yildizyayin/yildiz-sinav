...
export async function refreshStudentIntelligence(env:Env,studentId:string){
 ...
 const targets=await all<any>(env.DB.prepare(`SELECT id,target_type,priority,motivation_label future_identity_label,created_at FROM student_academic_targets WHERE student_id=? AND status='ACTIVE' ORDER BY priority,created_at`).bind(studentId));
