...
async function grantMembership(request:Request,env:Env,user:AuthUser):Promise<Response>{
  if(user.role!=='SUPER_ADMIN')return forbidden();const b=await requestBody(request);if(!b.studentId||!b.planId)return badRequest('Öğrenci ve plan gereklidir.');const id=uuid('mem');const plan=await one<any>(env.DB.prepare(`SELECT * FROM membership_plans WHERE id=? OR code=?`).bind(b.planId,b.planId));if(!plan)return notFound('Plan bulunamadı.');
  await env.DB.batch([env.DB.prepare(`UPDATE student_memberships SET status='EXPIRED' WHERE student_id=? AND status='ACTIVE'`).bind(b.studentId),env.DB.prepare(`INSERT INTO student_memberships(id,student_id,plan_id,status,starts_at,ends_at,source) VALUES(?,?,?,'ACTIVE',CURRENT_TIMESTAMP,?,?)`).bind(id,b.studentId,plan.id,b.endsAt||null,b.source||'ADMIN'),env.DB.prepare(`INSERT INTO live_credit_wallets(student_id,balance) VALUES(?,?) ON CONFLICT(student_id) DO UPDATE SET balance=balance+excluded.balance,updated_at=CURRENT_TIMESTAMP`).bind(b.studentId,Number(plan.monthly_live_credits||0)),env.DB.prepare(`INSERT INTO live_credit_ledger(id,student_id,amount,reason,reference_id) VALUES(?,?,?,?,?)`).bind(uuid('lcd'),b.studentId,Number(plan.monthly_live_credits||0),'MEMBERSHIP_GRANT',id)]);return json({ok:true,id});
}
...
async function games(request:Request,env:Env,user:AuthUser):Promise<Response>{
  const gate=await requireFeature(env,user,'GAMES');if(gate)return gate;const u=new URL(request.url);const sid=await scopedStudentId(env,user,u.searchParams.get('studentId'));if(!sid)return forbidden();if(request.method==='GET'){const profile=await one<any>(env.DB.prepare(`SELECT * FROM gamification_profiles WHERE student_id=?`).bind(sid));const achievements=await all<any>(env.DB.prepare(`SELECT a.*,sa.earned_at FROM student_achievements sa JOIN achievements a ON a.id=sa.achievement_id WHERE sa.student_id=? ORDER BY sa.earned_at DESC`).bind(sid));return json({ok:true,profile:profile||{student_id:sid,xp:0,level:1,streak_days:0},achievements,games:[{code:'MATH_SPRINT',title:'Hızlı Matematik'},{code:'WORD_HUNT',title:'Kelime Avı'},{code:'PARAGRAPH_SPRINT',title:'Paragraf Sprint'},{code:'SCIENCE_MATCH',title:'Fen Eşleştirme'}]});}const b=await requestBody(request);const xp=Math.max(0,Math.min(500,Number(b.xpEarned||0)));const id=uuid('game');await env.DB.batch([env.DB.prepare(`INSERT INTO game_sessions(id,student_id,game_code,node_id,score,xp_earned,duration_seconds,payload_json) VALUES(?,?,?,?,?,?,?,?)`).bind(id,sid,b.gameCode||'GENERIC',b.nodeId||null,Number(b.score||0),xp,b.durationSeconds||null,JSON.stringify(b.payload||{})),env.DB.prepare(`INSERT INTO gamification_profiles(student_id,xp,level,streak_days,last_activity_date,updated_at) VALUES(?,?,1,1,date('now'),CURRENT_TIMESTAMP) ON CONFLICT(student_id) DO UPDATE SET xp=xp+excluded.xp,level=1+CAST((xp+excluded.xp)/1000 AS INTEGER),streak_days=CASE WHEN last_activity_date=date('now','-1 day') THEN streak_days+1 WHEN last_activity_date=date('now') THEN streak_days ELSE 1 END,last_activity_date=date('now'),updated_at=CURRENT_TIMESTAMP`).bind(sid,xp)]);return json({ok:true,id,xpEarned:xp});
}
