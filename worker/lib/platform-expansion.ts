...
async function contentOptions(request:Request,env:Env,user:AuthUser):Promise<Response>{
  ...
  const nodes=grade&&subjectId?await all<any>(env.DB.prepare(`SELECT id,code,title,node_type,parent_id FROM learning_nodes WHERE active=1 AND grade_level=? AND subject_id=? AND node_type IN ('TOPIC','SUBTOPIC','OUTCOME','SKILL') ORDER BY node_type,title LIMIT 600`).bind(grade,subjectId)):[];
  ...
  return json({ok:true,subjects,nodes,institutions,canCreate:['SUPER_ADMIN','INSTITUTION_MANAGER','TEACHER','GUIDANCE_TEACHER'].includes(user.role),canReview:user.role==='SUPER_ADMIN'});
}
