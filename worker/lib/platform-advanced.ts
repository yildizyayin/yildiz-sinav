...
async function premiumAi(request:Request,env:Env,user:AuthUser,mode:'GUIDANCE'|'COACH'){
  ...
  if(!answer.startsWith('Nibiru'))answer=`Nibiru · ${label}: ${answer}`;
...
async function personalBook(request:Request,env:Env,user:AuthUser){
  ...
  if(weak.length){const marks=weak.map(()=>'?').join(',');qs=await all<any>(env.DB.prepare(`SELECT DISTINCT q.id FROM question_bank q JOIN question_learning_links l ON l.question_id=q.id WHERE q.review_status='APPROVED' AND l.node_id IN (${marks}) ORDER BY COALESCE(q.difficulty_level,q.difficulty,3),q.created_at DESC LIMIT ?`).bind(...weak.map(x=>x.node_id),count));}
